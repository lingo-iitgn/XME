...
def heatmap(x_data, name="out", axis=0, cbar=True, cbar_ax=None, xticklabels=True, yticklabels=True, title="", dir="horizontal"):
    ...
    if dir == "horizontal":
        ...
        g = sns.heatmap(tab, xticklabels=col_name, annot=tab, vmin=70, vmax=100, cmap="Blues", fmt=".2f", ax=axis, cbar=cbar, cbar_ax=cbar_ax if cbar else None)
        if yticklabels:
        ...
        # Change yticklabel size to 20
        # plt.yticks(fontsize=20)
        g = sns.heatmap(tab, annot=tab, vmin=70, vmax=100, cmap="Blues", fmt=".2f", ax=axis, cbar=cbar, cbar_ax=cbar_ax if cbar else None, cbar_kws={"orientation": "horizontal"})
        if xticklabels:
            g.set_xticklabels(col_name, fontsize=16)
...
if TYPE == "lang":
    ...
    for ax, model in zip(axes[:-1], MODELS):
        ...
        wl_mat = []
        for lang in langs:
            es = to_matrix(f"./loc/in-{ALGO}-{model}/" + lang + ".txt")
    ...
    fig.savefig(f"./out/{ALGO}-wl-loc.pdf")
    ...
    fig.savefig(f"./out/{ALGO}-wl-mi-loc.pdf")
