...
def loc_acc(pre_logits, post_logits):
    assert pre_logits.shape == post_logits.shape, "Shape Mismatch of logits in loc_acc"
    pre_logits = pre_logits.to(torch.float32)
    post_logits = post_logits.to(torch.float32)

    # Coverrting logits to probabilities
    pre_probs = pre_logits.softmax(-1)
    post_probs = post_logits.softmax(-1)

    # Counting the number of correct predictions
    correct = (pre_probs.argmax(-1) == post_probs.argmax(-1)).float().mean()
