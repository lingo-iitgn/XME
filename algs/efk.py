...
from transformers import BartForConditionalGeneration, T5ForConditionalGeneration, XLMRobertaForSequenceClassification, BertForSequenceClassification, BloomForSequenceClassification
    ...
    def __init__(self, model, config, model_constructor, editor=None):
        ...
        if editor is None:
            if isinstance(model, BertClassifier):
                ...
                # XLM-Roberta
                embedding = model.roberta.embeddings.word_embeddings.weight.data
            elif isinstance(model, BertForSequenceClassification):
                # mBERT
                embedding = model.bert.embeddings.word_embeddings.weight.data
            elif isinstance(model, BloomForSequenceClassification):
                # BLOOM
                embedding = model.transformer.word_embeddings.weight.data
            else:
