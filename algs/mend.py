...
def update_counter(x, m, s, k):
    # print(x.shape, m.shape, s.shape, k.shape)
    new_m = m + (x - m) / k
    new_s = s + (x - m) * (x - new_m)
...
class MEND(EditableModel):
    def get_shape(self, p):
        ...
        # return p.shape if isinstance(self.model, transformers.GPT2LMHeadModel) else (p.shape[1], p.shape[0]) # GPT fc
        return p.shape if isinstance(self.model, transformers.GPT2LMHeadModel) or isinstance(self.model, transformers.GPT2ForSequenceClassification) else (p.shape[1], p.shape[0])
    ...
    def edit(self, batch, condition=None, detach_history=False):
        ...
        if isinstance(self.model, transformers.GPT2LMHeadModel) or isinstance(self.model, transformers.GPT2ForSequenceClassification):
            targ = "ij"
        else:
            targ = "ji"
        mean_grads = {
