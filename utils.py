...
class RunningStatAverager:
    ...
    def average(self):
        average = {}
        # print(self.underlying.keys())
        for k, v in self.underlying.items():
            if not k.startswith("nll/") and not k.startswith("all_lang/"):
                average[k] = sum(v) / len(v)
            elif k.startswith("all_lang/loc"):
                langs = set()
                for row_vals in self.underlying[k]:
                    for lang, val in row_vals.items():
                        langs.add(lang)
                        if "loc_"+lang not in average:
                            average["loc_"+lang] = 0.0
                            average["loc_len_"+lang] = 0
                        average["loc_"+lang] += val.item() # Val is a tensor
                        average["loc_len_"+lang] += 1
                
                for lang in langs:
                    average["loc_"+lang] /= average["loc_len_"+lang]

            elif k.startswith("all_lang/edit"):
                langs = set()
                for row_vals in self.underlying[k]:
                    for lang, val in row_vals.items():
                        langs.add(lang)
                        if "es_"+lang not in average:
                            average["es_"+lang] = 0.0
                            average["es_len_"+lang] = 0
                        average["es_"+lang] += val["acc"].item() # Val is a tensor
                        average["es_len_"+lang] += 1

                for lang in langs:
                    average["es_"+lang] /= average["es_len_"+lang]
